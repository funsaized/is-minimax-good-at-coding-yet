...
const PIECES: { id: 'is' | WordId | 'frontend'; text: Record<VoiceId, string>; marked: boolean; glyph?: string }[] = [
    ...
    <section className="press reveal" id="press" aria-labelledby="press-title">
      <header className="press__header">
        ...
        <p className="section__lede">
          The lever on the left cycles the line into the next voice. The composing stick in the middle shows
          the marked and unmarked pieces. The sheet on the right is the pulled impression. Click the knob,
          or press <kbd>shift</kbd>+<kbd>v</kbd>.
        </p>
      ...
      <div className={`press__table press__table--${voice} ${pulling ? 'is-pulling' : ''}`}>
        <div className="press__cell press__cell--lever">
          <span className="press__cell-key">the lever</span>
          <div className="press__cell-body">
            ...
            >
              ...
              <span className="press-lever__label" aria-hidden="true">pull</span>
              <span className="press-lever__hint" aria-hidden="true">
                click · or <kbd>shift</kbd>+<kbd>v</kbd>
              </span>
            </button>

            <div className="press-state" aria-live="polite">
              <div className="press-state__voice">
                <span aria-hidden="true">{VOICE_LETTER[voice]}</span>
                <span>{VOICE_NAME[voice]}</span>
              </div>
              <span className="press-state__meta">{VOICE_FACE[voice]}</span>
              <div className="press-state__next">
                <span className="press-state__next-rule" aria-hidden="true" />
                next · {VOICE_NAME[next]}
              </div>
            </div>
          </div>
        </div>

        <div className="press__cell press__cell--staff">
          <div className="press-staff">
            <div className="press-staff__pinion" aria-hidden="true">
              <span className="press-staff__pinion-rule" />
              the composing stick
            </div>

            <div className="press-staff__row" aria-label="The line, set three ways">
              {PIECES.map(piece => {
                const isActive = piece.id === word
                return (
                  <span
                    key={piece.id}
                    className={`press-staff__token ${piece.marked ? 'is-marked' : ''} ${isActive ? 'is-active' : ''}`}
            ...
            <div className="press-staff__readout">
              <span className="press-staff__readout-mark" aria-hidden="true">
                {word === 'm3' ? '⌇' : word === 'good' ? '∧' : '?'}
              </span>
              <span className="press-staff__readout-row">
                <strong>{PROOF[word].label}</strong>
                <em>{PROOF[word].sub}</em>
              </span>
              <span className="press-staff__readout-meta">
                active piece · № {(['m3', 'good', 'yet'] as WordId[]).indexOf(word) + 1}/3
              </span>
            </div>
          </div>
        </div>
      ...
      <p className="press__caption">
        Each pull cycles through <em>quiet cut</em> → <em>human hand</em> → <em>bold signal</em>. The button
        stays bound to the title above and the type plate below.
      </p>
    </section>
  )
}
