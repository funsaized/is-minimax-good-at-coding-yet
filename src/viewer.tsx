...
export function Viewer() {
  ...
  return <main className={`observatory ${expanded ? 'expanded' : ''}`}>
    ...
    <section className="machine" aria-label="Iteration viewer">
      ...
      <div className="stage">
        ...
        <div className="page-frame">
          {version ? <>
            {(scrub !== null || !frameReady) && <picture><source media="(max-width: 560px)" srcSet={version.mobileScreenshot} /><img className="frame-preview" src={version.screenshot} alt={`Preview of iteration ${version.id}`} /></picture>}
    ...
    <section className="timeline" aria-label="Time machine controls">
      <div className="timeline-heading"><div><History size={17} /><h2>A little time travel.</h2><span className="timeline-hint">Drag to see how we got here</span></div><span className="archive-label">THE ARCHIVE</span></div>
      <div className="transport">
        ...
        <div className="slider-wrap"><input type="range" min="0" max={Math.max(0, last)} value={Math.max(0, selected)} disabled={versions.length < 2} aria-label="Iteration timeline" aria-valuetext={version ? `Iteration ${version.id}: ${version.summary}` : 'No iterations yet'} style={{ '--progress': `${last > 0 ? selected / last * 100 : 0}%` } as React.CSSProperties} onPointerDown={e => { e.currentTarget.setPointerCapture(e.pointerId); setPlaying(false); setScrub(Math.max(0, selected)) }} onChange={e => { const value = Number(e.target.value); setPlaying(false); if (scrub !== null) setScrub(value); else select(value) }} onPointerUp={e => { select(Number(e.currentTarget.value)); setScrub(null) }} onPointerCancel={() => setScrub(null)} onBlur={() => { if (scrub !== null) select(scrub); setScrub(null) }} /><div className="slider-labels"><span>000 · THE BEGINNING</span><span>{last >= 0 ? number(versions[last].id) : '000'} · NOW</span></div></div>
        ...
        <button className={`live-button ${live ? 'is-live' : ''}`} onClick={goLive}><Radio size={14} />{live ? 'Latest' : 'Back to latest'}{!live && selected < last && <span className="new-count">+{last - selected}</span>}</button>
      </div>
